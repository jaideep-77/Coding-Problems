...
class TranscriptTester {
	final int RUBRIC = 12;
	Transcript expected_tr = new Transcript("input.txt", "output.txt");
	Transcript actual_tr = new Transcript("input.txt", "output.txt");
	Transcript expected_wrong_tr = new Transcript("wrongInput.txt", "wrongOutput.txt"); // Note to myself: make wrongInput wrong!
	Transcript actual_wrong_tr = new Transcript("wrongInput.txt", "wrongOutput.txt"); 
	...
	void buildStudentArray1() {
		...
		try {
			expected = expected_tr.buildStudentArray();
		...
		try {
			actual = actual_tr.buildStudentArray();
		} catch (InvalidTotalException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private static boolean studentEqual(ArrayList<Student> actual, ArrayList<Student> expected) {
		boolean equal = true; 
	...
	void buildStudentArray2() throws InvalidTotalException {
		ArrayList<Student> expected = expected_tr.buildStudentArray();
	...
	public void testweightedGPA1() throws InvalidTotalException {
		Transcript expected = new Transcript("myInput1.txt", "outputTranscript.txt");
		Student stExpected = expected.buildStudentArray().get(0);
	...
	public void testweightedGPA2() throws InvalidTotalException {
		Transcript expected = new Transcript("myInput2.txt", "outputTranscript.txt");
		Student stExpected = expected.buildStudentArray().get(1);
	...
	public void testAddCourse1() {
		ArrayList<Assessment> as_expected = new ArrayList<Assessment>(); 
		as_expected.add(Assessment.getInstance('p', 50));
		as_expected.add(Assessment.getInstance('e', 50));
		...
		Course crs_expected = new Course("E", as_expected, 3.0);
		Course crs_actual = new Course("E", as_actual, 3.0);
		Student expected_st = new Student(); 
		Student actual_st = new Student();
		expected_st.addCourse(crs_expected);
		actual_st.addCourse(crs_actual);
		ArrayList<Course> crsList_expected = expected_st.getCourseTaken();
	...
	public void testAddCourse2() {
		ArrayList<Assessment> as_expected = new ArrayList<Assessment>(); 
		as_expected.add(Assessment.getInstance('p', 50));
		as_expected.add(Assessment.getInstance('e', 40));
		as_expected.add(Assessment.getInstance('p', 10));
